...
const LocatePatientsPage = () => {
  ...
  const [lastNotificationTime, setLastNotificationTime] = useState(null); // Track last notification time
      ...
      try {
        ...
        if (data && typeof data.latitude === 'number' && typeof data.longitude === 'number') {
          ...
          if (data.inSafetyZone === false) {
            ...
            // Send a request to create the notification in the backend if within time window
  ...
  const notifyPatientOutOfZone = async (patientId) => {
    ...
    const windowTime = 30 * 60 * 1000; // 30 minutes window in milliseconds

    // If there's no last notification time or if enough time has passed, send the notification
    if (!lastNotificationTime || (currentTime - lastNotificationTime) > windowTime) {
      try {
        ...
        if (response.ok) {
          ...
          setLastNotificationTime(currentTime); // Save the time of this notification
        } else {
      ...
      console.log("Notification skipped. Time window not yet passed.");
    }
  };
